...
class FinalMigration:
    ...
    def load_embedding_model(self, model_name="sentence-transformers/all-mpnet-base-v2"):
        """Load SentenceTransformer model for embeddings.
        
        The 'all-mpnet-base-v2' model produces 768-dimensional embeddings.
        We'll pad these to 1536 dimensions to match the database schema.
        """
        try:
            logger.info(f"Loading embedding model: {model_name}")
            self.sentence_transformer = SentenceTransformer(model_name)
            self.embedding_dimension = 768  # Base dimension for this model
        ...
        try:
            ...
            if cursor.fetchone() is None:
                ...
                try:
                    ...
                    self.pg_conn.rollback()  # Make sure to rollback on error
    ...
    def build_clustering_data(self, sample_size=10000):
        ...
        try:
            ...
            # Create a new connection for positions to avoid cursor issues
            self.mysql_conn.close()
            self.connect_mysql()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Final MySQL to PostgreSQL Migration')
